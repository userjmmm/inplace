...
describe('좋아요/취소 기능 테스트', () => {
  ...
  const testLikeButtonFunctionality = async (likeButton: HTMLElement) => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('PiHeartFill')).not.toBeInTheDocument();
      expect(screen.getByTestId('PiHeartLight')).toBeInTheDocument();
    });
  };

  function InfluencerTestWrapper() {
    const { data } = useQuery({
    ...
    if (!data) return null;
    return <InfluencerItem {...data[0]} />;
  }

  test('인플루언서 좋아요/좋아요 취소가 반영되는지 확인', async () => {
    renderWithProviders(<InfluencerTestWrapper />);
  ...
  test('장소 좋아요/좋아요 취소가 반영되는지 확인', async () => {
    renderWithProviders(<PlaceTestWrapper />);
